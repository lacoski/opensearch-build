...
RUN ls -l $TEMP_DIR && \
    ...
    # Fix High CVEs (GHSA-72hv-8253-57qq, CVE-2025-54920, etc.)
    dnf install -y findutils unzip zip && \
    curl -L https://repo1.maven.org/maven2/com/fasterxml/jackson/core/jackson-core/2.20.2/jackson-core-2.20.2.jar -o /tmp/jackson-core-2.20.2.jar && \
    curl -L https://repo1.maven.org/maven2/com/fasterxml/jackson/core/jackson-databind/2.20.2/jackson-databind-2.20.2.jar -o /tmp/jackson-databind-2.20.2.jar && \
    curl -L https://repo1.maven.org/maven2/com/fasterxml/jackson/core/jackson-annotations/2.19.0/jackson-annotations-2.19.0.jar -o /tmp/jackson-annotations-2.19.0.jar && \
    ...
    find $OPENSEARCH_HOME -name "jackson-core-2.*.jar" -exec sh -c 'cp /tmp/jackson-core-2.20.2.jar $(dirname $1)/jackson-core-2.20.2.jar && rm $1' _ {} \; && \
    find $OPENSEARCH_HOME -name "jackson-databind-2.*.jar" -exec sh -c 'cp /tmp/jackson-databind-2.20.2.jar $(dirname $1)/jackson-databind-2.20.2.jar && rm $1' _ {} \; && \
    find $OPENSEARCH_HOME -name "jackson-annotations-2.*.jar" -exec sh -c 'cp /tmp/jackson-annotations-2.19.0.jar $(dirname $1)/jackson-annotations-2.19.0.jar && rm $1' _ {} \; && \
